...
class Bacon_Ipsum {
	 ...
	 * @var      string
	 */
	protected $plugin_slug = 'bacon-ipsum';
	/**
	 * @var      object
	 */
...
// Add shortcodes
		add_shortcode('baconipsum', array($this, 'render_element'));
		add_shortcode('bacon_mockup', array($this, 'render_element'));
	...
	function render_shortcode_panel($shortcode, $type = 1, $template = false){
		...
		if(!empty($template)){
			echo "<script type=\"text/html\" id=\"bacon-ipsum-".$shortcode."-config-tmpl\">\r\n";
		}
		echo "<input id=\"bacon-ipsum-shortcodekey\" class=\"configexclude\" type=\"hidden\" value=\"".$shortcode."\">\r\n";
		echo "<input id=\"bacon-ipsum-shortcodetype\" class=\"configexclude\" type=\"hidden\" value=\"".$type."\">\r\n";
		echo "<input id=\"bacon-ipsum-default-content\" class=\"configexclude\" type=\"hidden\" value=\" ".__('Your content goes here','bacon-ipsum')." \">\r\n";
		...
		$groups = array();
		echo "<div class=\"bacon-ipsum-shortcode-config-nav\">\r\n";
		echo "	<ul>\r\n";
		foreach ($configfiles as $key=>$fieldfile) {
			include $fieldfile;
			$groups[] = $group;
		...
		echo "<div class=\"bacon-ipsum-shortcode-config-content " . ( count($configfiles) > 1 ? "" : "full" ) . "\">\r\n";
			foreach($groups as $key=>$group){
				echo "<div class=\"group\" " . ( $key === 0 ? "" : "style=\"display:none;\"" ) . " id=\"row".$group['master']."\">\r\n";
				echo "<h3 class=\"bacon-ipsum-config-header\">".$group['label']."</h3>\r\n";
				...
				if(!empty($group['multiple'])){
					echo "<div class=\"toolrow\"><button class=\"button bacon-ipsum-add-group-row\" type=\"button\" data-rowtemplate=\"group-".$group['id']."-tmpl\">".__('Add Another','bacon-ipsum')."</button></div>\r\n";
				}
				echo "</div>\r\n";
			}
		echo "</div>\r\n";
	...
	function shortcode_modal_template(){
		...
		echo "<script type=\"text/html\" id=\"bacon-ipsum-shortcode-panel-tmpl\">\r\n";
		echo "	<div tabindex=\"0\" id=\"bacon-ipsum-shortcode-panel\" class=\"hidden\">\r\n";
		echo "		<div class=\"media-modal-backdrop\"></div>\r\n";
		echo "		<div class=\"bacon-ipsum-modal-modal\">\r\n";
		echo "			<div class=\"bacon-ipsum-modal-content\">\r\n";
		echo "				<div class=\"bacon-ipsum-modal-header\">\r\n";
		echo "					<a title=\"Close\" href=\"#\" class=\"media-modal-close\">\r\n";
		echo "						<span class=\"media-modal-icon\"></span>\r\n";
		echo "					</a>\r\n";
		echo "					<h2 style=\"background: url(".self::get_url( '/assets/images/icon.png', __FILE__ ) . ") no-repeat scroll 0px 2px transparent; padding-left: 20px;\">".__('Bacon Ipsum','bacon-ipsum')." <small>".__("Shortcode Builder","bacon-ipsum")."</small></h2>\r\n";
		echo "				</div>\r\n";
		echo "				<div class=\"bacon-ipsum-modal-body\">\r\n";
		echo "					<span id=\"bacon-ipsum-categories\">\r\n";
		echo "						Shortcode: <span id=\"bacon-ipsum-elements\"><select class=\"bacon-ipsum-elements-selector\" id=\"bacon-ipsum-elements-selector\"><option value=\"\">Select Shortcode</option><option value=\"baconipsum\">".__('Bacon Ipsum Builder','bacon-ipsum')."</option>
<option value=\"bacon_mockup\">".__('Bacon Mockup','bacon-ipsum')."</option>
</select>
</span>
\r\n";
		echo "					</span>\r\n";
		echo "					<div id=\"bacon-ipsum-shortcode-config\" class=\"bacon-ipsum-shortcode-config\">\r\n";
		echo "					</div>\r\n";
		echo "				</div>\r\n";
	...
	public function get_post_meta($id, $key = null, $single = false){
		...
		if(!empty($key)){
			...
			if(file_exists(self::get_path( __FILE__ ) .'configs/fieldgroups-bacon_ipsum.php')){
				include self::get_path( __FILE__ ) .'configs/fieldgroups-bacon_ipsum.php';		
			}else{
				return;
			}
	...
	function save_post_metaboxes($pid, $post){
		...
		if(!isset($_POST['bacon_ipsum_metabox']) || !isset($_POST['bacon_ipsum_metabox_prefix'])){return;}


		if(!wp_verify_nonce($_POST['bacon_ipsum_metabox'], plugin_basename(__FILE__))){
			return $post->ID;
		}
		...
		foreach( $_POST['bacon_ipsum_metabox_prefix'] as $prefix ){
			if(!isset($_POST[$prefix])){continue;}

			
			delete_post_meta($post->ID, $prefix);
			add_post_meta($post->ID, $prefix, $_POST[$prefix]);
			//foreach($_POST['bacon_ipsum'] as $field=>$data){
				
				//if(is_array($data)){
				//	foreach($data as $item){
				//		add_post_meta($post->ID, 'bacon_ipsum_'.$field, $item);
				//	}
				//}else{
	...
	public function render_element($atts, $content, $slug, $head = false) {
		...
		if(!empty($head)){
			$instanceID = $this->element_instance_id('bacon_ipsum'.$slug, 'header');
		}else{
			$instanceID = $this->element_instance_id('bacon_ipsum'.$slug, 'footer');
		}
		...
		foreach ($wp_query->posts as $key => &$post) {
			...
			if(!empty($shortcodes[2])){
				foreach($shortcodes[2] as $foundkey=>$shortcode){
					...
					wp_enqueue_script( 'bacon', false, false , false, false );
