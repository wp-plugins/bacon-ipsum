...
function bacon_ipsum_init_slider(){
	jQuery('.slider').remove();
	jQuery('.bacon_ipsum_simple_slider').each(function(k,v){
		var slide = jQuery(v);
		slide.simpleSlider({
...
jQuery(function($){
	//bacon_ipsum_field_callbacks.push('bacon_ipsum_init_slider');
	bacon_ipsum_init_slider();
	$(document).ajaxSuccess(function(e, xhr, settings) {
		if(settings.data){
			if(settings.data.search('action=save-widget') != -1 && settings.data.search('bacon_ipsum') != -1) {
				bacon_ipsum_init_slider();
			}
		}
	});
